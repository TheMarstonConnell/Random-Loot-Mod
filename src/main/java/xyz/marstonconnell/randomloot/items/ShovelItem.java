...
public class ShovelItem extends ItemSpade implements RandomTool {
	...
	public ShovelItem(int variants) {
		super(ToolMaterialList.random, 2, 1, new Properties());
		this.variants = variants;
		rand = new Random();
		randomizeVariant();
		setRegistryName(RandomLoot.MODID, "shovel");
...
//			lore.add(new NBTTagString(TextFormatting.DARK_RED + "Auto-Smelt"));
	...
	public float getDestroySpeed(ItemStack stack, IBlockState state) {
		if (getToolTypes(stack).stream().anyMatch(e -> state.isToolEffective(e)))
			return efficiency;
		return ShovelItem.EFFECTIVE_ON.contains(state.getBlock()) ? RLUtils.getRLMDigSpeed(stack) : 1.0F;
	}
